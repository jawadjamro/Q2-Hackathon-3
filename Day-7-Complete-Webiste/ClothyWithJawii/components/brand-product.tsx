...
import { brandProductsQuery } from '@/sanity/lib/queries';
...
export default function BrandProducts() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      const products: Product[] = await client.fetch(brandProductsQuery);
                  ...
                  <div className="relative aspect-square bg-slate-50  rounded-[4px] sm:p-4">
                    {product.tags.includes('brand') && (
                      <div className="absolute top-2 sm:top-3 left-2 sm:left-3 z-10">
                        <span className="bg-orange-600 text-black text-[10px] sm:text-xs px-2 sm:px-3 py-1 rounded-[4px]">
                          Brand
                        </span>
                      </div>
                    )}
